...
import com.eagle.mailer.entity.sequence.SenderSequenceEntity;
import lombok.AllArgsConstructor;


@Component
@AllArgsConstructor
public class SenderSequenceRepository {
	 ...
	 private final MongoOperations mongo;
	    ...
	    public BigInteger getNextSequence() {
	        return mongo.findAndModify(query(where("_id").is(SEQUENCE_NAME)), new Update().inc("seq", 1),
	                options().returnNew(true).upsert(true), SenderSequenceEntity.class).getSeq();
	    }
